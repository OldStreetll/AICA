...
export function nextjsFileToRouteURL(filePath) {
    ...
    if (appMatch) {
        ...
        if (!stripped.startsWith('api/') && stripped !== 'api')
            return null;
        return '/' + stripped;
    }
...
export function normalizeFetchURL(rawURL) {
...
export function routeMatches(fetchURL, routeURL) {
    ...
    if (lastRoutePart?.startsWith('[...') || lastRoutePart?.startsWith('[[...')) {
        ...
        if (fetchParts.length < routeParts.length - 1)
            return false;
        return routeParts.slice(0, -1).every((part, i) => {
            if (part.startsWith('[') || fetchParts[i].startsWith('['))
                return true;
            return part === fetchParts[i];
        });
    }
    if (fetchParts.length !== routeParts.length)
        return false;
    return fetchParts.every((part, i) => {
        if (part.startsWith('[') || routeParts[i].startsWith('['))
            return true;

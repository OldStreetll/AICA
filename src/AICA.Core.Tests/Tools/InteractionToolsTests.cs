...
namespace AICA.Core.Tests.Tools
{
    public class InteractionToolsTests
    {
        ...
        #region UpdatePlanTool Tests

        [Fact]
        public async Task UpdatePlanTool_WithValidPlan_UpdatesContext()
        {
            // Arrange
            var tool = new UpdatePlanTool();
            var context = CreateMockContext();
            var uiContext = CreateMockUIContext();

            var planSteps = new[]
            {
                new { step = "Step 1", status = "completed" },
                new { step = "Step 2", status = "in_progress" },
                new { step = "Step 3", status = "pending" }
            };
            ...
            var call = new ToolCall
            {
                ...
                Arguments = new Dictionary<string, object>
                {
                    ...
                    ["plan"] = JsonSerializer.Serialize(planSteps)
                }
            };
            ...
            // Assert
            Assert.True(result.Success);
            Assert.Contains("Plan updated", result.Content);
            context.Verify(c => c.UpdatePlan(It.IsAny<TaskPlan>()), Times.Once);
        }

        [Fact]
        public async Task UpdatePlanTool_WithMissingPlan_ReturnsFail()
        {
            // Arrange
            var tool = new UpdatePlanTool();
            var context = CreateMockContext();
            ...
            // Assert
            Assert.False(result.Success);
            Assert.Contains("Missing required parameter: plan", result.Error);
        }

        [Fact]
        public void UpdatePlanTool_GetMetadata_ReturnsCorrectCategory()
        {
            // Arrange
            var tool = new UpdatePlanTool();
            ...
            Assert.NotNull(metadata);
            Assert.Equal(ToolCategory.Interaction, metadata.Category);
            Assert.Equal("update_plan", metadata.Name);
        }

        #endregion

        #region AttemptCompletionTool Tests

        [Fact]
        public async Task AttemptCompletionTool_WithValidResult_ReturnsSuccess()
        {
            // Arrange
            var tool = new AttemptCompletionTool();
            var context = CreateMockContext();
            var uiContext = CreateMockUIContext();
            var call = new ToolCall
            {
            ...
            // Assert
            Assert.True(result.Success);
            Assert.Contains("Task completed successfully", result.Content);
        }

        [Fact]
        public async Task AttemptCompletionTool_WithCommand_IncludesCommand()
        {
            // Arrange
            var tool = new AttemptCompletionTool();
            var context = CreateMockContext();
        ...
        [Fact]
        public async Task AttemptCompletionTool_WithMissingResult_ReturnsFail()
        {
            // Arrange
            var tool = new AttemptCompletionTool();
            var context = CreateMockContext();
            ...
            // Assert
            Assert.False(result.Success);
            Assert.Contains("Missing required parameter: result", result.Error);
        }

        [Fact]
        public async Task AttemptCompletionTool_WithEmptyResult_ReturnsFail()
        {
            // Arrange
            var tool = new AttemptCompletionTool();
            var context = CreateMockContext();
        ...
        [Fact]
        public void AttemptCompletionTool_GetMetadata_ReturnsCorrectCategory()
        {
            // Arrange
            var tool = new AttemptCompletionTool();
            ...
            Assert.NotNull(metadata);
            Assert.Equal(ToolCategory.Interaction, metadata.Category);
            Assert.Equal("attempt_completion", metadata.Name);
            Assert.True(metadata.RequiresApproval);
        }

        #endregion

        #region CondenseTool Tests

        [Fact]
        public async Task CondenseTool_WithValidSummary_ReturnsSuccess()
        {
            // Arrange
            var tool = new CondenseTool();
            var context = CreateMockContext();
            var uiContext = CreateMockUIContext();
            var call = new ToolCall
            {
                ...
                Arguments = new Dictionary<string, object>
                {
                    ["summary"] = "Work completed so far: read files, made edits, ran tests"
                }
            };
            ...
            // Assert
            Assert.True(result.Success);
            Assert.Contains("CONDENSE:", result.Content);
        }

        [Fact]
        public async Task CondenseTool_WithMissingSummary_ReturnsFail()
        {
            // Arrange
            var tool = new CondenseTool();
            var context = CreateMockContext();
            ...
            // Assert
            Assert.False(result.Success);
            Assert.Contains("Missing required parameter: summary", result.Error);
        }

        [Fact]
        public async Task CondenseTool_WithEmptySummary_ReturnsFail()
        {
            // Arrange
            var tool = new CondenseTool();
            var context = CreateMockContext();
        ...
        [Fact]
        public void CondenseTool_GetMetadata_ReturnsCorrectCategory()
        {
            // Arrange
            var tool = new CondenseTool();
            ...
            Assert.NotNull(metadata);
            Assert.Equal(ToolCategory.Interaction, metadata.Category);
            Assert.Equal("condense", metadata.Name);
        }

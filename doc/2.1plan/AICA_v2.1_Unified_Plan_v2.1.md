# AICA v2.1 统一实施方案（最终版）

> **版本**：v2.1
> **日期**：2026-04-06
> **基于**：v1.0 方案 + 双实例联合审查（严格审查 + 可行性分析）+ 两轮双实例联合审查（4份独立报告 + 2份联合讨论）
> **约束**：单人开发、VS2022 VSIX、MiniMax-M2.5 单模型、涉密离线环境
> **核心策略**：弱模型 + 强系统
> **排期**：24 周目标 / 26 周上限（v1.0: 16周 → v2.0: 22-24周 → v2.1: 24-26周）
...
| 修订 | v1.0 | v2.0 | 理由 |
|------|------|------|------|
| 排期 | 16 周 / 6 Phase | 24-26 周 / 8 Phase + 3 弹性窗口 | 两份报告独立估算均为 21-26 周 |
...
| SK 步骤 8 | Phase 1 直接执行 | **条件性执行，等基线数据就绪** | 需要 T1 Telemetry 前提 |
| T2 会话摘要 | Phase 7 | **收尾阶段** | 优先级低于 Phase 7 核心任务，移至收尾 |
| M2（搁置） | v1.0 计划中 | **搁置** | v1.0 原 M2 在合并整理时移除，编号保留以避免混淆 |
| OH4（搁置） | v1.0 计划中 | **搁置** | v1.0 原 OH4 在合并整理时移除，编号保留以避免混淆 |
...
| S2 | 编辑后自动构建 | 扬长板 | 5-7 天 | Phase 5 | **从 Phase 6 前移**；工作量上调（首个 fire-and-forget 模式） |
...
| T2 | Telemetry 会话摘要 | Harness 新增 | 2-3 天 | 收尾 | 从 Phase 7 移至收尾阶段 |
...
```
无依赖（可独立开始）：
  T1-infra, M1, M3, SK, H1, H2, H3a, OH2, S3

有依赖：
  H3b 权限决策持久化 → H3a 权限反馈注入（需要先有反馈注入机制才能持久化决策）
  OH5 SubAgent → 建议在 H1 之后（ReviewAgent 需要截断持久化支持完整输出审查）
  OH3 Agent Hook → OH5 SubAgent（需要 ReviewAgent）
  PA1 PlanAgent 优化 → OH5 SubAgent（需要 SubAgent 基类重构后再改 PlanAgent；Phase 5 内部 OH5 先于 PA1）
  S1 符号检索 → SK（需要意图分类）
...
**v2.1 补充：AppendDiagnosticsAsync 迁移**

现有 `AppendDiagnosticsAsync`（EditFileTool.cs line 401）必须迁移为 PostEditPipeline 的默认 step：

```csharp
// DiagnosticsStep — 迁移自 AppendDiagnosticsAsync
public class DiagnosticsStep : IPostEditStep
{
    public string Name => "Diagnostics";
    public bool IsEnabled => true;  // 始终启用，非 feature flag 控制
    public int Order => 900;        // 所有其他 step 之后
    // ...
}
```

**PostEditContext 最小字段定义（v2.1 新增）**：

```csharp
public class PostEditContext
{
...
EditFileTool 有三种编辑模式，PostEditPipeline 必须在每种模式的成功路径后调用：

| 模式 | 代码位置 | 调用点 |
|------|---------|--------|
| 单文件编辑 | line ~319 | 替换 AppendDiagnosticsAsync |
| 多编辑点（同文件多处修改） | line ~589 | 替换 AppendDiagnosticsAsync |
| 多文件编辑 | line ~647 | 每个文件成功编辑后调用一次 |
...
| Phase 1 | DiagnosticsStep (现有迁移) | pipeline 末尾 (Order=900) | 迁移 AppendDiagnosticsAsync |
...
|  | 7. **FF1 框架搭建（v2.1 新增）**：在 AicaConfig 中添加 `features` section + 读取逻辑，为后续所有 Feature flags 提供基础 |
...
|  | 8. ~~`DynamicToolSelector.cs` 去掉工具过滤~~ **条件性执行**：等 T1 基线数据采集 2 周后（约 Phase 2 开始时）评估是否去掉过滤，对比幻觉率变化后决定。如果基线数据显示工具过滤有效（幻觉率低），此步骤将不执行。 |
| **技能激活机制（v2.0 明确）** | |
...
> **弹性周设计（v2.1）**：前 2 天收集用户反馈 + 做决策；后 3 天用于消化前序 Phase 溢出 / 补充集成测试 / 提前启动下一 Phase 准备工作。
...
**并行策略**：S3 从第 6 周起与 H1 工具接入阶段并行启动（详见第五节周计划）。
...
|  | 4. **混合截断策略（v2.0 新增）**：Phase 2 实现通用 PersistAndTruncate 基础设施 + "use read_file" 提示；**构建输出的智能错误提取在 S2（Phase 5）上线时一并实现**，届时对 RunCommandTool 的编译器输出自动提取错误/警告行注入 ToolResult |
|  | 5. 后台清理：7 天过期自动删除 |
...
| **验收标准** | 所有工具截断后完整输出可通过 read_file 访问；7 天自动清理（构建输出智能提取在 S2 Phase 5 上线后验收） |
...
- 集中式截断服务，所有工具统一接入
- 通用截断持久化（手动回读提示；智能错误提取随 S2 Phase 5 上线）
- 头文件同步检测（C/C++ 杀手级功能）
...
#### PA1：PlanAgent 输出优化（2-3 天）← 依赖 OH5 SubAgent 基类
...
#### S2：编辑后自动构建（5-7 天）← 从 Phase 6 前移；工作量上调（fire-and-forget 模式设计成本）
...
- T2 Telemetry 会话摘要（2-3天）+ 全量集成测试（特别是 EditFileTool 7 项修改的回归测试）
- Bug 修复与性能优化
- 文档更新
- 6 名试用用户的最终验收反馈
...
第 16 周 ──── OH5 ReviewAgent PoC (2d) → PA1 PlanAgent 优化 (3d)  [串行：PA1 依赖 SubAgent 基类]
第 17 周 ──── S2 后台构建 (5d)
...
**每周额外隐含工作**：T1 telemetry 埋点（~0.5d）+ AS1 假设记录（~0.25d）+ FF1 Feature flag（~0.25d）。横切项工作量由验证窗口（弹性周）和各 Phase 内缓冲天数吸收，未在各任务工作量中单独计入。
...
| **Evaluation** | ❌ 缺 Agent 级 | ⚠️ ReviewAgent PoC + S3 头文件同步 + S4 Impact 预警 + S2 后台构建（四层评估）^[1]^ |
| **Memory** | ⚠️ 全量拼接 | ✅ 相关性检索 + Skills 数据驱动 + 快照安全网 |

> ^[1]^ Evaluation 维度标注 ⚠️ 而非 ✅：ReviewAgent PoC 需在 Phase 5 验证窗口确认 MiniMax 审查能力，采纳率 >30% 方可扩展。四层评估中仅 ReviewAgent 存在不确定性，其余三层（S3/S4/S2）为确定性程序化检测。

> 各维度的量化验收指标见第四节各 Phase 交付物和验收标准，此表为定性概览。
...
| MCP-B 行为规则注入改变 LLM 行为 | 工具调用频率/响应模式变化 | 验证窗口 2 中 A/B 对比测试；config 开关可禁用规则注入 | §9 MCP-B |
...
| 第二轮严格审查 | `doc/2.1plan/review_v2_strict_audit.md` |
| 第二轮可行性分析 | `doc/2.1plan/review_v2_feasibility.md` |
| 第二轮联合结论 | `doc/2.1plan/review_v2_joint_conclusion.md` |
...
| MCP 整合讨论记录 | `doc/discussion/SK_MCP_Integration_Discussion.md` |
| MCP 冗余文件问题 | `doc/nextstep2.1/AICA_MCP_Redundant_Files_Issue.md` |

## 九、MCP 内容结构化方案（2026-04-07 追加）

> **来源**：6 Pane 并行讨论（Pane0 总指挥 + Pane1-4 开发 + Pane5 审核），讨论产出见 `doc/discussion/` 目录
> **关键发现**：Pane5 审核发现数据源假设错误，经源码验证确认，改变了 MCP-B 方向

### 9.1 关键事实（经源码验证）

| 事实 | 源码位置 | 说明 |
|------|---------|------|
| `gitnexus://setup` 返回**工具参考表格** | `resources.js:385-419 getSetupResource()` | 从数据库动态生成，含 7 工具 + 资源列表 |
| AGENTS.md 文件包含**行为规则** | `ai-context.js:46-109 generateGitNexusContent()` | Always/Never/When Debugging/When Refactoring/Self-Check |
| 两者是**独立的内容生成路径** | `resources.js` vs `ai-context.js` | 数据源不同、内容不同 |
| AICA 当前只读 MCP resource | `AgentExecutor.cs:1033` | `client.ReadResourceAsync("gitnexus://setup")` |
| **行为规则从未被注入 AICA prompt** | — | AICA 不读文件系统 AGENTS.md，行为规则一直缺失 |

### 9.2 决策汇总

| ID | 方案 | 决策 | 理由 | 工作量 |
|----|------|------|------|--------|
| **MCP-A** | 冗余文件清理 | **立即执行** | Pane3 方案 A2（后置清理），0 依赖，解决用户痛点 | 0.5 天 |
| **MCP-B** | 行为规则注入 | **内置规则文件**（替代原 McpRuleAdapter） | 数据源假设错误使原方案失效；Pane5 替代方案更简单 | 0.5 天 |
| **MCP-C** | McpResourceResolver | **不做** | YAGNI，低频场景工具调用已覆盖 | — |
| **MCP-D** | McpServerDescriptor | **不做** | YAGNI，单 MCP 服务器不需通用框架 | — |

### 9.3 MCP-A：冗余文件清理（Pane3 方案 A2）

| 项 | 内容 |
|----|------|
| **改什么** | `GitNexusProcessManager.cs` |
| **怎么改** | analyze 进程成功退出后（exitCode==0），调用 `CleanupRedundantFiles(repoRoot)` 删除 AGENTS.md、CLAUDE.md、.claude/ 目录 |
| **改两处** | `TriggerIndexAsync`（line ~298 后）+ `TriggerIndexWithProgressAsync`（line ~387 后） |
| **新增代码** | ~30 行私有方法，含异常捕获和日志 |
| **排期** | Phase 2 R2 间隙，随下次提交一起 |
| **验收标准** | analyze 完成后项目目录中不残留 AGENTS.md/CLAUDE.md/.claude/；清理失败不影响索引功能 |

#### MCP-A 测试清单（6人审查 F3 补充）

**T1 文件检查**
- [ ] analyze完成后AGENTS.md不存在
- [ ] analyze完成后CLAUDE.md不存在
- [ ] analyze完成后.claude/目录不存在

**T2 MCP可用性**
- [ ] 清理后gitnexus://setup仍返回正确工具参考表格
- [ ] 清理后GitNexus MCP工具调用（如detect_changes）正常工作

**T3 回归7项**
- [ ] 索引功能正常（analyze进程正常完成）
- [ ] analyze进程退出码正确传播
- [ ] 日志输出包含清理操作记录
- [ ] 异常路径：清理失败时analyze结果不受影响（fail-open）
- [ ] 多仓库场景：每个仓库独立清理
- [ ] 首次索引：无冗余文件时CleanupRedundantFiles幂等无报错
- [ ] 重复索引：已清理过的仓库再次analyze不产生错误

### 9.4 MCP-B：GitNexus 行为规则内置化

**方案**：将 `ai-context.js:46-109` 中的行为规则直接作为 `.aica-rules/` 内置规则文件，利用现有 RuleLoader + RuleEvaluator 自动加载和条件激活。

**不做 McpRuleAdapter 的原因**：
1. `gitnexus://setup` 不包含行为规则（数据源假设错误）
2. 行为规则在 `ai-context.js` 中是静态模板，不需要动态解析
3. 内置规则文件复用全部 SK 基础设施，零架构新增
4. 工作量从 2-3 天降至 0.5 天

#### 规则文件清单（5 个 .md 文件）

基于 `ai-context.js:46-109` 的内容，创建以下内置规则文件：

| 文件名 | 对应段落 | type | intent | 注入条件 |
|--------|---------|------|--------|---------|
| `gitnexus-always.md` | Always Do（5 条） | `rule` | 无 | 无条件，每次注入 |
| `gitnexus-never.md` | Never Do（4 条） | `rule` | 无 | 无条件，每次注入 |
| `gitnexus-debugging.md` | When Debugging（4 步） | `rule` | `bug_fix` | 仅 bug_fix intent |
| `gitnexus-refactoring.md` | When Refactoring（3 条） | `rule` | `refactor` | 仅 refactor intent |
| `gitnexus-self-check.md` | Self-Check（4 项） | `rule` | 无 | 无条件，每次注入 |

#### 规则文件 frontmatter 格式示例

```yaml
---
name: gitnexus-always
description: GitNexus mandatory behaviors — always run impact analysis before edits
type: rule
source: builtin
priority: 0
---
...
带 intent 条件的示例（`gitnexus-debugging.md`）：

```yaml
---
name: gitnexus-debugging
description: GitNexus debugging workflow — systematic steps when investigating bugs
type: rule
intent: bug_fix
source: builtin
priority: 0
...
> **RuleSource 优先级决策（6人审查 F4 补充）**：所有 `gitnexus-*.md` 内置规则使用 `priority: 0`（Builtin 级别，最低优先级），而非 `Mcp=15`。理由：GitNexus 行为规则是通用指导性质，用户全局规则（Global=10）和工作区规则（Workspace=20）应能覆盖。`Builtin=0` 语义更准确——内容来自内置文件而非 MCP 动态获取，与 Pane1 原设计 `Mcp=15` 不同。

#### 与现有 `gitnexus://setup` 注入的关系

| 内容 | 来源 | 注入方式 | 变更 |
|------|------|---------|------|
| 工具参考表格 | `gitnexus://setup` MCP resource | `InjectMcpResources()` → `AddMcpResourceContext()` | **保持不变** |
| 行为规则 | `.aica-rules/gitnexus-*.md` 内置文件 | RuleLoader → RuleEvaluator → SystemPromptBuilder | **新增** |

**注意**：这意味着行为规则是**首次**被注入 AICA prompt。需要测试注入后 MiniMax-M2.5 是否正确响应这些规则指导（特别是 impact analysis 和 detect_changes 的调用频率变化）。

#### intent 条件激活的前置条件

**前置条件 1：ClassifyIntent() 需支持 `refactor`（6人审查 F1 补充）**

当前 `DynamicToolSelector.ClassifyIntent()` 返回值集合为 `bug_fix`/`modify`/`read`/`analyze`/`command`/`conversation`/`general`，不包含 `refactor`。这意味着 `gitnexus-refactoring.md`（intent=`refactor`）永远不会被激活。

| 方案 | 描述 | 优缺点 |
|------|------|--------|
| **A（推荐）** | ClassifyIntent() 增加 `refactor` 子分类（从 `modify` 拆分，识别重构关键词） | 语义更精确，重构规则仅在真正重构时注入 |
| **B（备用）** | `gitnexus-refactoring.md` 改为 `intent=modify` | 零改动成本，但重构规则在所有 modify 场景都会注入 |

**决策**：采用方案 A。在 Phase 2 MCP-B 实现时同步修改 ClassifyIntent()。

**前置条件 2：RuleEvaluator 需增加 intent 过滤**

`When Debugging` 和 `When Refactoring` 规则需要 intent 匹配。当前 `RuleEvaluator.EvaluateRules()` 只做 path 匹配，不处理 intent。需要在 `EvaluateRules()` 方法内部（`RuleEvaluator.cs`）增加 intent 过滤逻辑（~10 行），与 F2 隔离表格中 `type="rule"` 的处理路径保持一致：

```csharp
// 对 type="rule" 的规则增加 intent 过滤
var universalRules = rules.Where(r => string.IsNullOrEmpty(r.Metadata?.Intent));
var intentRules = rules.Where(r => !string.IsNullOrEmpty(r.Metadata?.Intent)
...
**intent 过滤注入区域隔离（6人审查 F2 补充）**

| 组合 | 处理路径 | 注入区域 |
|------|---------|---------|
| `type="rule"` + intent | `EvaluateRules()` 内过滤 | `## GitNexus Guidelines` |
| `type="skill"` + intent | `AddSkillsByIntent()` 内过滤 | `## Task Template` |

`AddSkillsByIntent()` 已通过 `EvaluateSkillsByIntent()` 显式过滤 `type="skill"`（`RuleEvaluator.cs:96`），只处理技能规则。当前 `EvaluateRules()` **不区分 type**（`RuleEvaluator.cs:70-82`），所有 enabled 且匹配 path 的规则都会被选中。MCP-B 新增的 5 个规则文件均为 `type="rule"`，不会被 `EvaluateSkillsByIntent()` 重复选中，因此不存在交叉注入问题。但为确保与上表的隔离规则完全一致，**MCP-B 实现时需在 `EvaluateRules()` 中增加 `type="rule"` 过滤条件**（~2 行）。

| 项 | 内容 |
|----|------|
| **改什么** | 新增 5 个 `.aica-rules/gitnexus-*.md` 文件 + `RuleEvaluator.cs` 增加 intent 过滤（~10 行） |
| **排期** | 验证窗口 2 弹性时间（第 11 周），不影响主线 |
| **验收标准** | GitNexus 行为规则通过 SK 机制自动注入 prompt；intent 条件规则仅在匹配时激活；`gitnexus://setup` 工具参考注入不受影响 |

### 9.5 排期影响

| 任务 | 排期位置 | 对主线影响 |
|------|---------|-----------|
| MCP-A 冗余文件清理 | Phase 2 R2 间隙（当前） | 无，0.5 天间隙工作 |
| MCP-B 行为规则内置化 | 验证窗口 2 弹性时间（第 11 周） | 无，使用弹性缓冲 |
| MCP-C/D | 不做 | 无 |
| **总计额外工作量** | **~1 天** | **零主线影响** |

### 9.6 Pane1 设计的复用

虽然 McpRuleAdapter 类不再需要，Pane1 的以下设计成果可直接复用：

| 成果 | 复用方式 |
|------|---------|
| Rule 属性映射表（五类规则 → YAML frontmatter） | 直接用于编写 `.aica-rules/*.md` 文件的 frontmatter |
| RuleSource 优先级设计 | 内置规则使用 `Builtin=0`（最低优先级），用户可通过 Workspace 规则覆盖。**决策理由**：GitNexus 行为规则是通用指导，非 MCP 动态获取，`Builtin=0` 语义准确且允许用户覆盖（区别于 Pane1 原设计 `Mcp=15`） |
| intent 过滤逻辑 | 直接在 RuleEvaluator 中实现 |
| 错误处理原则（fail-open） | 适用于 RuleLoader 加载内置文件的异常处理 |

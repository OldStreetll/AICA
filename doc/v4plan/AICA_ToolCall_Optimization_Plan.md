...
> **[2026-03-27 确认] 全部纳入 WorkPlan 阶段 3 执行，不再推迟。**

```
Phase A → WorkPlan 步骤 3.9（+1 天）:
  方向 3: 移除优先策略偏见（5 行删除）          ← 已复测验证方向正确 [C92 覆盖 C91]
  方向 1: 去除 DynamicToolSelector 工具过滤     ← 改动小，OpenCode 验证过

Phase B → WorkPlan 步骤 3.10 Part 2（+2 天）:
  方向 2: 移除 AddToolDescriptions              ← 需隔离测试，避免重蹈 C86 覆辙
  方向 4: 消除原生描述竞态                       ← 改善首次请求体验

Phase C → WorkPlan 步骤 3.10 Part 3（+3 天）:
  方向 5: 精简 System Prompt                    ← 方向 1-4 完成后执行，逐条测试
```
...
> **[2026-03-27 更新] 5 个方向全部纳入 WorkPlan 执行计划，不再推迟。** 原 C91"优先策略暂保留"决定已被 C92 覆盖。

...
| C30 | general.md C# 规则移除 | 移除 C# XML 文档注释规则，改为语言中立表述指向 doxygen | AICA 仅服务 C/C++ 开发者，不需要 C# 规则处理 |
...
| C33 | 规范文件技术债务 | 记录：5 个 cpp-*.md 从 .doc 原始提取，MISRA C 依赖 AI 知识库而非逐条核对 | 源文档未经标准化格式输入，待 M1 前审查补漏（步骤 2.1 期间穿插进行 [C50]） |
...
| F8 修改影响预警 | M2 | 步骤 2.1 GitNexus impact() | 平台组+界面组 ~40 人 [C39] | ⚠️ 依赖 GitNexus，不可用时从开源生态找替代 [C13] |

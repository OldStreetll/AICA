...
### v2.0 修正（非正式部署问题收集与复现）

| # | 修正项 | 决定 | 原因 |
|---|--------|------|------|
| C75 | 非正式部署问题追踪 | 新增"非正式部署问题追踪"章节（阶段 2 末尾），结构化记录 `67a79c0` 版本暴露的问题 | 6 人试用期间反馈了多个问题，需逐条收集、复现、定位根因、确认修复状态 |
| C76 | 用最新版本复现旧版问题 | 开发者在最新代码（`aa05da6`，含 GitNexus + 规范注入重构 + 数据目录统一）上复现 `67a79c0` 的问题，修复直接进入主线 | `67a79c0` 后的 7 个 commit 均为增量叠加（无破坏性重构），修复在最新代码上进行避免分支分叉。部分旧版问题可能已被后续改动顺带修复 |
...
> 进度：✅ **阶段 2 全部完成（2026-03-27）。** 步骤 2.1 GitNexus 集成 ✅ + 步骤 2.2 自测 ✅ + 非正式部署问题修复 D-01~D-06 ✅ + 步骤 2.3 M1 验收通过。残留问题 D-07~D-09（MEDIUM/HIGH）纳入阶段 3 对应步骤解决
...
> 部署版本：`67a79c096ef6f29dc4d1fb3764306fd1e7a1edc8`（阶段 0+1 成果）
...
> - MEDIUM/LOW：记录后纳入步骤 3.9（M1 反馈修复窗口）
> - 若问题已被 `67a79c0` 之后的 commit 顺带修复，标注"最新版已修复"并注明修复 commit
> - 所有 CRITICAL/HIGH 收敛后方可推进步骤 2.3 扩大试用
>
> **修复优先级（2026-03-26 讨论确认）：**
>
> ```
> 立即修复（阻塞试用）：
>   1. D-04 推理过滤器误判           ← CRITICAL，改动小，立即修复
...
> 前置：阶段 2 完成
>
> **⚠️ [C80/C81] 步骤 3.5（H7 迭代预算感知）和步骤 3.6（H6 Condense 保护区）已提前到阶段 2 实施**，用于修复非正式部署问题 D-01 和 D-02。阶段 3 执行时这两步已完成，直接进入步骤 3.7。
...
3.4 状态机 (H2) [C5/C6 修正]  ─→ 3.5 迭代预算感知 (H7) ⚠️ 已提前到阶段 2 [C80]

── 上下文 + 记忆 ──────────────────────────────────────────────
3.6 Condense 保护区 (H6) ⚠️ 已提前到阶段 2 [C81] ─→ 3.7 断点续做
                                                            │
                                                            ▼
                                                      3.8 Memory Bank
...
### 步骤 3.5：H7 迭代预算感知 ⚠️ 已提前到阶段 2 [C80]

> **[C80] 本步骤已提前到阶段 2 实施**，作为 D-01（迭代耗尽）修复方案的一部分，与 GitNexus 优先策略组成双层方案。
...
### 步骤 3.6：H6 Condense 保护区 ⚠️ 已提前到阶段 2 [C81]

> **[C81] 本步骤已提前到阶段 2 实施**，作为 D-02（跨任务上下文污染）修复方案。Condense 时区分当前任务关键信息和历史任务摘要，新任务只保留历史极简版本。

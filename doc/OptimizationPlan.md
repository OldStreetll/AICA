...
- **[P0-007] 流式输出被 completion 覆盖 → ✅ 已修复**
  - 修复: ChatToolWindowControl 新增 `preToolContent` 缓冲区，ToolStart 时保存流式文本再清除，Complete 时恢复
  - 重测: E01 private 方法完整覆盖(28个)，E03 完整重构代码可见。E 类全部 PASS
- **[P1-017] 工具调用失败/叙述阻塞 → 部分修复**
  - 修复: AgentExecutor 叙述指纹检测(重复≥2次 force-complete) + OpenAIClient partial tool call 恢复
  - 重测: D02 CalcManager 问题不再悬挂，正常终止+⚠️警告。LLM 偶发不调用工具属 LLM 行为
...
- ~~TC-E01/E03~~ — **P0-007 已修复并重测通过** ✅ E01 完整性 B+→A，E03 PARTIAL PASS→PASS

...
| | | | | |
| **E 类: 右键命令功能** | | | | |
| TC-E01 | A- | **A** | B+ | **PASS** ⚠️P0-007 |
| TC-E02 | **A** | **A** | **A-** | **PASS** |
| TC-E03 | B+ | B | C+ | PARTIAL PASS ⚠️P0-007 |
| TC-E04 | A- | B+ | **A** | **PASS** |
...
> **注意**: 此用例的评分可能偏低，因为 P0-007（流式输出丢失）导致 LLM 实际生成的详细内容无法被看到。修复 P0-007 后应重测。

### 发现的问题

1. **[P0-007] 流式输出内容被 completion 覆盖（严重）**: AICA 在流式输出阶段产生了详细内容，但 `attempt_completion` 触发后全部消失。用户只能看到 completion 卡片的简短摘要。TC-E01 也有相同现象。这是 UI 层 bug，可能在 ChatToolWindowControl 或 AgentExecutor 的 completion 处理逻辑中。
2. **[P1-018] Refactor 命令 completion 未包含具体重构代码**: completion 卡片仅 6 条文字摘要，无可直接应用的代码。可能与 P0-007 相关（代码在流式输出中已生成但被覆盖）。
3. **[P2-010] 重构建议未考虑项目代码风格**: ratpak 是 C 风格遗留代码库，引入 C++ RAII 和命名空间与整体风格不一致。
...
1. **[P1-017] 工具调用失败/叙述性文本阻塞**: AICA 在第 2 轮 "CalcManager 模块负责什么？" 连续两次输出叙述文本（"让我读取一些关键文件..."）但未实际调用工具，也未调用 attempt_completion，导致任务悬挂。用户被迫跳过。违反了 "NO narration" 和 "Call tools IMMEDIATELY" 规则。可能原因：(1) LLM 流式输出中断 (2) 工具调用被截断 (3) 上下文中缺少足够信息触发工具调用决策。
...
| P0-007 | TC-E01/E03 | **流式输出内容被 completion 覆盖**: LLM 流式输出的详细分析/代码在 attempt_completion 触发后完全消失，用户只能看到 completion 卡片的简短摘要。影响右键命令(Explain/Refactor)的用户体验。疑似 ChatToolWindowControl 或 AgentExecutor 的 completion 处理逻辑 bug | 待修复 |
...
| P1-017 | TC-D02 | 工具调用失败/叙述性文本阻塞：连续两次输出"让我读取..."但未调用工具或 completion，任务悬挂 | 待调查 |
| P1-018 | TC-E03 | Refactor 命令 completion 未包含具体重构代码：仅文字摘要。**可能受 P0-007 影响**（详细代码可能在流式输出中已生成但被 completion 覆盖） | LLM 行为 / P0-007 |
...
| P2-009 | TC-E01 | 私有方法完全未覆盖：~20 个 private 方法（含 ProcessCommandWorker、DoOperation）未分析。**可能受 P0-007 影响** | LLM 行为 / P0-007 |
...
|------|------|--------|-------------|--------|--------|----------|--------|
| P0 | 7 | **6** | 0 | 1 (P0-007) | 0 | 0 | 0 |
| P1 | 17 | **4** | 2 (P1-012/013) | 0 | 1 (P1-017) | 9 | 1 (P1-005) |
| P2 | 12 | **1** | 0 | 0 | 0 | 11 | 0 |
| **合计** | **36** | **11** | **2** | **1** | **1** | **20** | **1** |
...
| 维度 | E01 ⚠️ | E02 | E03 ⚠️ | E04 |
|------|---------|-----|---------|-----|
| 正确性 | A- | **A** | B+ | A- |
| 准确性 | **A** | **A** | B | B+ |
| 完整性 | B+ | A- | C+ | **A** |

> ⚠️ = 受 P0-007（流式输出丢失）影响，评分可能偏低，修复后需重测
...
**E 类测试（4/4 个用例已全部完成）：**
- **PASS**: E01/E02/E04 — 3 个用例
- **PARTIAL PASS**: E03 — 1 个用例（重构建议合理但 completion 无具体代码，受 P0-007 影响）
- E01: CCalcEngine 类解释 (A-/A/B+)，public 覆盖 96%
- E02: exprat() 算法解释 (A/A/A-)，数学原理精确
- E03: Refactor 建议方向正确但无代码 (B+/B/C+)，受 P0-007 流式输出丢失影响
- E04: Generate Test 生成 22 个测试 (A-/B+/A)，主动研究现有测试风格，覆盖 6 类场景
- E 类核心能力：代码解释(E01/E02) 和测试生成(E04) 优秀；重构(E03) 受 P0-007 影响评分偏低
...
| P0 bug 发现 | **7** 个: 6 已修复 + **1 待修复 (P0-007 流式输出丢失)** |
| P1 问题 | **17** 个: 4 已修复(含1 Prompt) + 2 已实施待重测 + 1 待调查 + 9 LLM行为 + 1 已关闭 |
| P2 问题 | **12** 个: 1 已修复(Prompt) + 11 LLM行为 |
...
- **[P0-007] 流式输出被 completion 覆盖**: E01/E03 的详细流式输出在 attempt_completion 后全部消失，影响右键命令体验
- **代码解释优秀，重构受 P0-007 影响**: E01/E02 解释能力 A 级；E03 completion 卡片无代码（流式阶段可能已生成）
...
| 优先级 | 问题 | 影响范围 | 修复方向 |
|--------|------|----------|----------|
| **P0** | P0-007 流式输出被 completion 覆盖 | E01/E03 及所有右键命令 | ChatToolWindowControl completion 处理逻辑：追加而非替换 |
| **P1** | P1-017 工具调用失败/叙述阻塞 | D02 | 调查 LLM 流式输出中断或工具调用解析失败 |
